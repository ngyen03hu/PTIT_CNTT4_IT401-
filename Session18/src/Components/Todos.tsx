...
//             case "EDIT":
//                 console.log("tien hanh edi");
//                 let index = state.job.findindex((ClipboardItem.job)=> ClipboardItem.id == useActionState.payload.id);
//                 state.job.splice(index,1,action,)
...
//     }
//     const editJod = (title:Job) => {
//         let new_title = prompt("Moi nhap them cong viec mowi");
//         let new_job = [...item.title.]
//     }
//     return (
//         <div>
...
//                 return <li key={index}>{item.title} <button onClick={() => deleteJob(item.id)}>Xóa </button><button onClick={() => editJod(item.title)}>chinh sua</button></li>
//             })}
//         </div>
//     )
// }